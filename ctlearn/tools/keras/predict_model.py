...
class PredictCTLearnKerasModel(Tool):
    """
    ...
    ``~dl1_data_handler.reader.DLDataReader`` and ``~ctlearn.core.keras.sequence.KerasSequence``.
    ...
    aliases = {
        ("i", "input_url"): "PredictCTLearnKerasModel.input_url",
        ("t", "type_model"): "PredictCTLearnKerasModel.load_type_model_from",
        ("e", "energy_model"): "PredictCTLearnKerasModel.load_energy_model_from",
        (
            "d",
            "cameradirection_model",
        ): "PredictCTLearnKerasModel.load_cameradirection_model_from",
        ("s", "skydirection_model"): "PredictCTLearnKerasModel.load_skydirection_model_from",
        ("o", "output"): "PredictCTLearnKerasModel.output_path",
    }
...
class MonoPredictCTLearnKerasModel(PredictCTLearnKerasModel):
    """
    Tool to predict the gammaness, energy and arrival direction from monoscopic R1/DL1 data using CTLearn models.

    This tool extends the ``PredictCTLearnKerasModel`` to specifically handle monoscopic R1/DL1 data. The prediction
    ...
    name = "ctlearn-predict-mono-keras-model"
    description = __doc__
    ...
    > ctlearn-predict-mono-model \\
        ...
        --PredictCTLearnKerasModel.batch_size=64 \\
        --PredictCTLearnKerasModel.dl1dh_reader_type=DLImageReader \\
...
class StereoPredictCTLearnKerasModel(PredictCTLearnKerasModel):
    """
    Tool to predict the gammaness, energy and arrival direction from R1/DL1 stereoscopic data using CTLearn models.

    This tool extends the ``PredictCTLearnKerasModel`` to specifically handle stereoscopic R1/DL1 data. The prediction
    is performed using the CTLearn models. The data is stored in the output file following the ctapipe DL2 data format.
    ...
    name = "ctlearn-predict-stereo-keras-model"
    description = __doc__
    ...
    > ctlearn-predict-stereo-model \\
        --input_url input.dl1.h5 \\
        --PredictCTLearnKerasModel.batch_size=16 \\
        --PredictCTLearnKerasModel.dl1dh_reader_type=DLImageReader \\
        --DLImageReader.channels=cleaned_image \\
        --DLImageReader.channels=cleaned_relative_peak_time \\
        --DLImageReader.image_mapper_type=BilinearMapper \\
...
def mono_tool():
    # Run the tool
    mono_tool = MonoPredictCTLearnKerasModel()
    mono_tool.run()


def stereo_tool():
    # Run the tool
    stereo_tool = StereoPredictCTLearnKerasModel()
    stereo_tool.run()
