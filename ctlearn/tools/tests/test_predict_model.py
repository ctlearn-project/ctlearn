...
def test_predict_mono_model_with_r1_waveforms(tmp_path, ctlearn_trained_r1_mono_models, r1_gamma_file):
    """
    ...
    for reco_task in ["type", "energy", "cameradirection"]:
        ...
        shutil.copy(ctlearn_trained_r1_mono_models[key], model_dir / f"ctlearn_mono_model_{key}.keras")
    ...
    with TableLoader(output_file, pointing=True, focal_length_choice="EQUIVALENT") as loader:
        ...
        tel_events = loader.read_telescope_events_by_id(telescopes=available_tels, dl1_parameters=True, dl2=True)
...
def test_predict_mono_model_with_dl1_images(tmp_path, ctlearn_trained_dl1_mono_models, dl1_gamma_file, dl2_tel_flag):
    """
    ...
    for telescope_type in telescope_types.keys():
        for reco_task in ["type", "energy", "cameradirection"]:
            key = f"{telescope_type}_{reco_task}"
            shutil.copy(ctlearn_trained_dl1_mono_models[key], model_dir / f"ctlearn_mono_model_{key}.keras")
            model_file = model_dir / f"ctlearn_mono_model_{key}.keras"
            assert model_file.exists(), f"Trained mono model file not found for {key}"
    # Build command-line arguments
    argv = [
    ...
    for telescope_type, allowed_tels in telescope_types.items():
        output_file = dl2_dir / f"gamma_{dl2_tel_flag}_{telescope_type}_mono_from_images.dl2.h5"
        # Run Prediction tool
        assert run_tool(
            MonoPredictCTLearnModel(),
            argv = argv + [
...
@pytest.mark.verifies_usecase("DPPS-UC-130-1.2.2")
def test_predict_stereo_model_with_dl1_images(tmp_path, ctlearn_trained_dl1_stereo_models, dl1_gamma_file):
    """
    ...
    for reco_task in ["type", "energy", "skydirection"]:
        key = f"{telescope_type}_{reco_task}"
        shutil.copy(ctlearn_trained_dl1_stereo_models[key], model_dir / f"ctlearn_stereo_model_{key}.keras")
        ...
        for tel_id in allowed_tels:
            ...
            for col in REQUIRED_COLUMNS + [
                ...
                if "CTLearnCameraReconstructor" in col:
                    col = col.replace("CTLearnCameraReconstructor", "CTLearnSkyReconstructor")
                assert col in tel_events[tel_id].colnames, f"{col} missing in DL2 file {output_file.name}"
                assert tel_events[tel_id][col][0] is not np.nan, f"{col} has NaN values in DL2 file {output_file.name}"
        # Check subarray-wise data
        subarray_events = loader.read_subarray_events(start=0, stop=2, dl2=True)
