...
"""
class QuantizePretrainedModel(CTLearnModel):
      
  #  ``LoadedModel`` is a pre-trained Keras model.

    This class extends the functionality of ``CTLearnModel`` by implementing
    methods to quantize the weights of a pre-trained Keras model.
    #
    ...
    quantize_head = Bool(
        default_value=False,
        allow_none=False,
        help="Set to set the fully connected head from the loaded model to be quantized.",
    ).tag(config=True)

    quantize_backbone = Bool(
        default_value=True,
        allow_none=False,
        help="Set the backbone model to be quantized.",
    ).tag(config=True)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.quantize_head:
            ...
            # Convert the model format to LiteRT
    ...
    def _build_quantized_backbone(self, input_shape):
       # 
        Build and quantize the LoadedModel backbone.
       ...
       # 
        ...
        for layer in self.model.layers:
            if layer.name.endswith("_block"):
                backbone_layer = self.model.get_layer(layer.name)
                self.backbone_name = backbone_layer.name
                backbone_layer.quantizable = self.quantizable_backbone
        network_output = backbone_layer(network_input)
        # Create the backbone model
        backbone_model = keras.Model(
            network_input, network_output, name=self.backbone_name
        ...
        # Prune the backbone
        converter.optimizations = [tf.lite.Optimize.DEFAULT]
        quant_backbone_model = converter.convert()
        
        return quant_backbone_model, network_input
