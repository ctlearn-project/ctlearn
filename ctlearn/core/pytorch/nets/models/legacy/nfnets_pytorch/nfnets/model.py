...
nfnet_params = {
    'F0': {
        ...
        'train_imsize': 192, 'test_imsize': 114,
...
# Definitions for VPGELU and VPReLU are assumed to be the same as above.

class NFNet(nn.Module):
    def __init__(self, num_channels=1, num_classes=2, variant='F0', stochdepth_rate=0.25, alpha=0.2, se_ratio=0.5, activation='gelu'):
        ...
        self.stem = Stem(num_channels=num_channels, activation=activation)

        # Here, we use a list to hold each NFBlock stage.
        blocks = []
        in_channels = block_params['width'][0] // 2

        # Adjust output sizes by maintaining depth but adjusting widths and strides.
        for i, (width, depth, stride) in enumerate(zip(block_params['width'], block_params['depth'], [2, 2, 2, 2])):
            out_channels = width
            for _ in range(depth):
                stride = stride if _ == 0 else 1
                blocks.append(NFBlock(in_channels=in_channels, out_channels=out_channels, stride=stride, se_ratio=se_ratio, activation=activation))
                in_channels = out_channels

        self.blocks = nn.Sequential(*blocks)

        # Adaptive pooling to adjust for any size discrepancies.
...
class Stem(nn.Module):
    def __init__(self, num_channels=1, activation='gelu'):
