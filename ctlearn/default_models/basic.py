...
def stacked_res_blocks(inputs, params):
    # Get custom hyperparameters
    residual_block = params['basic']['stacked_res_blocks'].get('residual_block', 'bottleneck')
    filters_list = [layer['filters'] for layer in
...
def basic_residual_block(inputs, filters, kernel_size=3, stride=1, conv_shortcut=True, attention=None, name=None):
    ...
    with tf.variable_scope("Basic_res_block"):
        ...
        x = tf.layers.conv2d(inputs, filters=filters, kernel_size=kernel_size,
                strides=stride, padding='same', activation=tf.nn.relu, name=name + '_1_conv')
        x = tf.layers.conv2d(x, filters=filters, kernel_size=kernel_size,
...
def bottleneck_residual_block(inputs, filters, kernel_size=3, stride=1, conv_shortcut=True, attention=None, name=None):
    ...
    with tf.variable_scope("Bottleneck_res_block"):
