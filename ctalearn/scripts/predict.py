...
def predict(config, test_data):
    ...
    if data_format == 'hdf5':
        ...
        test_generator = (
                ctalearn.data.get_data_generators_HDF5(data_files, metadata,
                    data_processing_settings, mode='test'))
    ...
    logger.info("Total number of test examples: {}".format(num_test_examples))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=("Predict on test data using a ctalearn model."))
    parser.add_argument(
        'config_file',
        help='config file used when training')
    parser.add_argument(
        'test_data',
        help='txt file list of HDF5 filenames for test data to predict on.')
    parser.add_argument(
        "--debug",
