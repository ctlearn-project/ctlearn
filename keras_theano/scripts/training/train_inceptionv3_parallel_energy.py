...
#samples
training_samples = (911227)
validation_samples = (496757)
...
def event_image_generator(path, batch_size):
    ...
    for i in range (0,79):
        bin_class = str(i)
        data_dir = os.path.join(path, bin_class)
        label = i

    for filename in os.listdir(data_dir):
        if filename.endswith(".png"):
            #get event ID and energy
            evnt_id, run_num, energy, impact, tel_num = filename.rsplit(".", 1)[0].split("_")
            event_id=evnt_id+'_'+run_num
            event_key = event_id'_' + bin_class
        ...
        for event_key in keys:
            ...
            for tel_num in ['T5','T6','T8','T9','T10','T11','T16','T17']:
                if tel_num in tels_dict:
                    img = load_image(os.path.join(path, bin_class, 
                        tels_dict[tel_num]))
                else:
                    img = load_empty()
                ...
                if tel_num == 'T5':
                    ...
                    X_train_T6[event_read_count % batch_size,:,:,:]dogsonly
                    = img
                elif tel_num == 'T8':
...
prediction = Dense(80, activation='softmax', name='prediction')(array_model)
